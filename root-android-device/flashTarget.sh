...
echo Starting to process device[$2] $1 at `date +%Y-%m-%d_%H:%M:%S` |& tee -a setup_device.log; 
...
fastboot reboot -s $1; 
echo Device[$2] $1 completed if no errors logged here, at `date +%Y-%m-%d_%H:%M:%S` |& tee -a setup_device.log;
